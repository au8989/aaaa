...
async function freeTrade(username: string, times: number) {
    for (let i = 0; i < times;) {
        ...
        try {
            tradeRef = await madfutClient.inviteWithTimeout(877769875686759659596756795678458675786957865876578658567856785678576678567885675678567867856785678, username, `Free trade for ${username} (${times - i} left)`);
            console.log(`${username} accepted invite.`);
        } catch {
...
bot.on("wallet", async (interaction, page) => {
    ...
    interaction.editOriginalMessage({
        embeds: [{
            color: 0x32A852,
            author: {
                name: `${interaction.member!.username}'s DiscFut wallet (page ${page}/${numPages})`,
...
bot.on("deposit", async (interaction, multiple) => {
    ...
    try {
        do {
            ...
            try {
                tradeRef = await madfutClient.inviteWithTimeout(username, 60000, "DiscFut Wallet deposit");
            } catch (err) {
...
async function withdraw(interaction: Eris.CommandInteraction<Eris.TextableChannel>, userId: string, username: string, coins: number, walletVerification: WalletVerificationResult) {
    ...
    while (coinsToGive > 0 || cardsToGive.size > 0 || packsToGive.size > 0) {
        const tradeRef = await madfutClient.inviteWithTimeout(username, 60000, "DiscFut Wallet withdrawal");
...
async function giveawayTrade(username: string) {
    while (giveawayRunning) {
        ...
        try {
            tradeRef = await madfutClient.inviteWithTimeout(username, 60000, "DiscFut giveaway 🎉");
        } catch {
            console.log(`${username} rejected invite or timed out.`);
            continue;
        }
